...
(() => {
  ...
  class DeckStage extends HTMLElement {
    ...
    _loadNotes() {
      ...
      if (!tag) { this._notes = []; this._broadcastNotes(); return; }
      try {
    ...
    _broadcastNotes() {
      // When embedded in an iframe (file://-safe), forward the notes catalog
      // to the parent so the host page can render a speaker-notes panel
      // without needing DOM access into the deck.
      if (!this._isFramed()) return;
      try {
        const labels = this._slides.map(s => s.getAttribute('data-label') || '');
        window.parent.postMessage({
          deckNotesReady: true,
          speakerNotes: (this._notes || []).slice(),
          deckLabels: labels,
    ...
    _applyIndex({ showOverlay = true, broadcast = true, reason = 'init' } = {}) {
      ...
      if (broadcast) {
        ...
        const payload = { slideIndexChanged: curr, deckTotal: this._slides.length, deckSkipped: this._skippedIndices() };
        try { window.postMessage(payload, '*'); } catch (e) {}
        // (1b) Also forward to the parent when embedded in an iframe.
        if (this._isFramed()) {
    ...
    _toggleSkip(i) {
      ...
      const payload = { slideIndexChanged: this._index, deckTotal: this._slides.length, deckSkipped: this._skippedIndices() };
      try { window.postMessage(payload, '*'); } catch (e) {}
      if (this._isFramed()) {
        try { window.parent.postMessage(payload, '*'); } catch (e) {}
      }
